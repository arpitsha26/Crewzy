...
function Post({ post }) {
    ...
    try {
      ...
      const updatedPosts=post.map(p=>p._id==post._id?updatedPost:p)
    ...
    <div className='w-[90%]  flex flex-col gap-[10px] bg-white items-center shadow-2xl shadow-[#00000058] rounded-2xl pb-[20px]'>
      ...
      <div className='w-full h-[60px] flex justify-between items-center px-[20px] mt-[10px]'>
        <div className='flex justify-center items-center gap-[10px]'>
            ...
            {!post.likes.includes(userData._id) && <GoHeart className='w-[25px] cursor-pointer h-[25px]'/>}
            {post.likes.includes(userData._id) && <GoHeartFill className='w-[25px] cursor-pointer h-[25px] text-red-600'/>}
      ...
      {showComment &&
        <div className='w-full  flex flex-col gap-[30px] pb-[20px]'>
          ...
          <button className='absolute right-[20px] cursor-pointer'><IoSendSharp className='w-[25px] h-[25px]'/></button>
          </div>
